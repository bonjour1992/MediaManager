...
function process(list)
{
	for ( var i in p)
	{
		if (list[0].prop.substring(1)==p[i]["alpha-2"])
		{
			var l = {id : p[i]["alpha-2"],name:p[i].name}
			db.save("nation",l,false,function ()
			{
				list.splice(0,1)
				process(list)
			})
	...
	console.log(list[0].prop.substring(1))
}
